...
import { Search, Play, Award, Zap, Layers, Eye, type LucideIcon } from "lucide-react";
...
const TAB_OPTIONS: { id: HubTab; label: string; Icon: LucideIcon }[] = [
  { id: "featured", label: "Featured", Icon: Award },
  { id: "most-watched", label: "Most Watched", Icon: Zap },
  { id: "newest", label: "Newest", Icon: Layers },
];

const SELECT_CLASS =
  "border border-gray-200 rounded-lg px-3 py-1.5 text-xs font-semibold bg-white focus:ring-2 focus:ring-[var(--hub-brand)] focus:outline-none";
...
function filterDemos(demos: SerializedDemo[], options: FilterOptions): SerializedDemo[] {
  const { activeTab, selectedIntegration, selectedRole, searchQuery } = options;
  let result = [...demos];
...
function CategoryTabs({
  ...
  return (
    <div className="flex items-center justify-between border-b border-gray-200 pb-2">
      ...
      <div className="text-xs font-semibold text-gray-400">Showing {resultCount} demos</div>
    </div>
  );
}

function DemoCard({ demo }: { demo: SerializedDemo }) {
  // Custom share link relative to current route
  const playLink = `/share/${demo.publicLink || demo.id}`;

  return (
    <div className="bg-white rounded-2xl border border-gray-150 overflow-hidden flex flex-col shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 group">
...
export default function HubClient({ settings, demos }: HubClientProps) {
  ...
  const filteredDemos = useMemo(
    () => filterDemos(demos, { activeTab, selectedIntegration, selectedRole, searchQuery }),
  ...
  return (
    ...
    >
      <HubHeader settings={settings} searchQuery={searchQuery} onSearchChange={setSearchQuery} />
